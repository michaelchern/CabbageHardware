...
{
    initialize(std::move(shaderCode), language, sourceLocation);
}

void ComputePipeline::initialize(std::string shaderCode, EmbeddedShader::ShaderLanguage language, const std::source_location &sourceLocation)
{
    EmbeddedShader::ShaderCodeCompiler shaderCodeCompilerLocal(shaderCode, EmbeddedShader::ShaderStage::ComputeShader, language, EmbeddedShader::CompilerOption(), sourceLocation);

    this->shaderCode = shaderCodeCompilerLocal.getShaderCode(EmbeddedShader::ShaderLanguage::SpirV);
    this->pushConstant = HardwarePushConstant(shaderCodeCompilerLocal.getShaderCode(EmbeddedShader::ShaderLanguage::SpirV).shaderResources.pushConstantSize, 0);

    shaderResource = shaderCodeCompilerLocal.getShaderCode(EmbeddedShader::ShaderLanguage::SpirV).shaderResources;
    initialized_ = true;
}
...
void ComputePipeline::executePipeline(ktm::uvec3 groupCount)
{
    if (!initialized_)
    {
        throw std::runtime_error("ComputePipeline not initialized. Call initialize(...) first or use the parameterized constructor.");
    }
